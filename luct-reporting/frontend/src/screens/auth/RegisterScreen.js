...
const ROLES = [
  ...
  { value: 'principal_lecturer', label: '🏛️ Principal Lecturer', desc: 'Review faculty reports' },
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const message = err.response?.data?.message
        || (err.request ? 'Cannot reach the backend server at http://10.150.234.158:5000. Make sure the backend is running and your phone is on the same Wi-Fi.' : err.message)
        || 'Please try again.';
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <View style={styles.headerBar}>
        ...
        <Text style={styles.headerTitle}>{step === 1 ? 'Select Role' : 'Create Account'}</Text>
        <View style={{ width: 38 }} />
      </View>
        ...
        {step === 1 ? (
          <View>
            ...
            {ROLES.map((r) => (
              <TouchableOpacity
                ...
                onPress={() => { update('role', r.value); setStep(2); }}
              >
                <Text style={styles.roleIcon}>{r.label}</Text>
                <Text style={styles.roleDesc}>{r.desc}</Text>
                {form.role === r.value && <Text style={styles.checkmark}>✓</Text>}
              </TouchableOpacity>
            ))}
          </View>
        ) : (
          <View style={styles.card}>
            <View style={styles.roleBadge}>
              <Text style={styles.roleBadgeText}>{ROLES.find(r => r.value === form.role)?.label}</Text>
            </View>

            <Input label="Full Name *" value={form.fullName} onChangeText={(v) => update('fullName', v)}
              placeholder="lisebo letele" error={errors.fullName} autoCapitalize="words" />
            
            <Input label="Email Address *" value={form.email} onChangeText={(v) => update('email', v)}
              placeholder="lisebo.letele@luct.edu.ls" keyboardType="email-address" autoCapitalize="none" error={errors.email} />

            <Input label="Faculty / Department *" value={form.facultyName} onChangeText={(v) => update('facultyName', v)}
              placeholder="e.g. Faculty of ICT" error={errors.facultyName} />

            {form.role === 'student' && (
              <Input label="Student ID" value={form.studentId} onChangeText={(v) => update('studentId', v)}
                placeholder="e.g. STU2024001" />
            )}

            {(form.role === 'lecturer' || form.role === 'principal_lecturer' || form.role === 'program_leader') && (
              <Input label="Staff ID" value={form.staffId} onChangeText={(v) => update('staffId', v)}
                placeholder="e.g. STAFF001" />
            )}

            {(form.role === 'program_leader' || form.role === 'principal_lecturer') && (
              <Input label="Program Name" value={form.programName} onChangeText={(v) => update('programName', v)}
                placeholder="e.g. BSc Software Engineering" />
            )}

            <Input label="Password *" value={form.password} onChangeText={(v) => update('password', v)}
              placeholder="Min 6 characters" secureTextEntry={!showPassword} error={errors.password}
              rightIcon={
            ...
            <Input label="Confirm Password *" value={form.confirmPassword} onChangeText={(v) => update('confirmPassword', v)}
              placeholder="Re-enter password" secureTextEntry error={errors.confirmPassword} />

            <Button title={loading ? "Creating account..." : "Create Account"} onPress={handleRegister} loading={false} disabled={loading} style={{ marginTop: 8 }} />
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  stepTitle: { fontSize: SIZES.xxl, fontWeight: '900', color: COLORS.gray800, marginBottom: 6, marginTop: 16 },
  ...
  card: { backgroundColor: COLORS.white, borderRadius: 20, padding: 20, marginTop: 8 },
  roleBadge: {
    backgroundColor: COLORS.primary + '12', borderRadius: 10, paddingHorizontal: 14,
    paddingVertical: 7, alignSelf: 'flex-start', marginBottom: 20,
  },
  roleBadgeText: { color: COLORS.primary, fontSize: SIZES.sm, fontWeight: '700' },
